...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["campaign_id"],
            ["postman_campaigns.id"],
        ),
        sa.ForeignKeyConstraint(
            ["contact_id"],
            ["live_chat_contacts.id"],
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        "postman_group_contacts",
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("contact_igs_id", sa.String(length=100), nullable=True),
        sa.Column("contact_id", postgresql.UUID(as_uuid=True), nullable=True),
    ...
    op.add_column(
        "academy_labels", sa.Column("name", sa.String(length=255), nullable=True)
    )
    op.drop_column("academy_labels", "label_name")
    op.add_column(
        "instagram_pages", sa.Column("username", sa.String(length=255), nullable=True)
    )
    op.add_column(
        "instagram_pages",
    ...
    op.add_column(
        "live_chat_contacts", sa.Column("message_count", sa.Integer(), nullable=True)
    )
    op.add_column(
        "live_chat_contacts", sa.Column("comment_count", sa.Integer(), nullable=True)
    )
    op.add_column(
        "live_chat_contacts",
...
def downgrade():
    ...
    op.add_column(
        "academy_labels",
        sa.Column(
            "label_name", sa.VARCHAR(length=255), autoincrement=False, nullable=True
        ),
    )
