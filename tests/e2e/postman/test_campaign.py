...
def test_create_campaign(
    ...
    group_id = group.id
    is_draft = True
    content = dict(title="pm", widget_type="MENU", type="WEB_URL",
                   choices=[{"text": "hello this is test!", "url": "test.com"}])

    r = client.post(
        f"{settings.API_V1_STR}/postman/campaign/", json={
            "name": name,
...
def test_get_campaigns(
    ...
    r = client.get(
        f"{settings.API_V1_STR}/postman/campaign/all", headers=headers
    )
    json_response = r.json()
    assert r.status_code == 200
