...
class InstagramGraphApi:
    ...
    def get_contact_information_from_facebook(
        ...
        if res.status_code == 200:
            ...
            # data_res = res.json()
            return dict(
                username=res.json()["username"] if "username" in res.json() else None,
                profile_image=res.json()["profile_pic"]
                if "profile_pic" in res.json()
                else None,
                name=res.json()["name"] if "name" in res.json() else None,
                followers_count=res.json()["follower_count"]
                if "follower_count" in res.json()
                else None,
                is_verified_user=res.json()["is_verified_user"]
                if "is_verified_user" in res.json()
                else None,
                is_user_follow_business=res.json()["is_user_follow_business"]
                if "is_user_follow_business" in res.json()
                else None,
                is_business_follow_user=res.json()["is_business_follow_user"]
                if "is_business_follow_user" in res.json()
                else None,
            )
