...
from app.core.telegram import admin_bot_handlers, bot_handlers, handlers, support_bot_handlers
...
def send_create_shop_notification_to_all_admins_task(shop_id, lang: str):
    db = SessionLocal()
    asyncio.run(
        admin_bot_handlers.send_create_shop_notification_to_all_admins_handler(db, shop_id, lang))
    db.close()
...
@celery.task
def send_lead_order_to_bot_and_support_bot_task(telegram_bot_id: int, lead_id: int,
                                                order_id: Optional[int], telegram_order_id: int,
                                                lang):
    db = SessionLocal()
    bot_message_id = None
    if lead_id:
        bot_message_id = asyncio.run(
            bot_handlers.send_lead_order_to_bot_handler(db, telegram_bot_id, lead_id, order_id,
                                                        lang))
...
def send_lead_pay_notification_to_support_bot_task(order_id: int, lang: str):
    db = SessionLocal()
    asyncio.run(
        support_bot_handlers.send_lead_pay_notification_to_support_bot_handler(db, order_id, lang))
...
def send_expiration_soon_notification_task():
    db = SessionLocal()
    asyncio.run(support_bot_handlers.send_expiration_soon_notification(
        db,
        "fa",
    ))
    db.close()
...
def set_all_bot_commands_task():
    db = SessionLocal()
    asyncio.run(bot_handlers.set_all_bot_commands_task_handler(db, "fa"))
    db.close()


@celery.task
def send_credit_extending_successful_notification_task(shop_credit_id,
                                                       shop_telegram_payment_record_id):
    db = SessionLocal()

    asyncio.run(
        support_bot_handlers.send_credit_extending_successful_notification_handler(
...
@celery.task
def send_lead_pay_notification_to_bot_task(order_id: int, lang: str):
    db = SessionLocal()

    asyncio.run(bot_handlers.send_lead_pay_notification_to_bot_handler(db, order_id, lang))
    db.close()
...
def order_change_status_from_dashboard_task(order_id, lang):
    db = SessionLocal()
    asyncio.run(bot_handlers.order_change_status_from_dashboard_handler(db, order_id, lang))
    db.close()
...
def send_shop_order_report_task(lang, shop_id, amount, currency, count, date):
    db = SessionLocal()
    asyncio.run(
        support_bot_handlers.send_shop_order_report(db, lang, shop_id, amount, currency, count,
                                                    date))
    db.close()
